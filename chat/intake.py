...
def bulk_extract(user_text: str, cs: dict, locale: str = "en") -> dict:
    ...
    system = (
        ...
        "Infer ONLY from a symptom of that kind. A generic failure with NO equipment named "
        "and no heat/water symptom — 'den startar inte', 'it won't start', 'den är trasig', "
        "'it doesn't work', 'den låter konstigt' — is NOT enough: return null. Guessing here "
        "put category='heat_pump' on a conversation about a lawnmower. Null when unsure.\n"
        ...
        "'what's the capital of France', a homework request). This includes a repair request "
        "for a DIFFERENT trade's machine — a car, a vehicle, a lawnmower, a computer — which "
        "is still off_domain even though it is a genuine repair request: 'kan ni laga min "
        "bil?' is off_domain true. A vague, garbled, or unclear "
