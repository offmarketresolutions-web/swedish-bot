...
def enrich_customer_from_session(customer, session) -> None:
    ...
    to the right records), captures the AI summary, and rebuilds the equipment summary.
    Call AFTER session.customer is set so the summary includes this session.
    """
    ...
    if session.ai_summary:
        customer.profile_summary = session.ai_summary
